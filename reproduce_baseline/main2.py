...
def train(model, loader, device, optimizer, epoch, label_names, current_alpha1):   #criterion with BCELoss
    model.train()
    total_loss = 0
...
def main():
    ...
    # Drop the 'descriptor' column, keep only label columns
    ...
    for fold, (train_idx, val_idx) in enumerate(mskf.split(smiles, labels), 1):
        ...
        print(model)
        optimizer = torch.optim.Adam(model.parameters(), weight_decay=1e-4, lr=1e-3)  # weight_decay=1e-4
        ...
        for epoch in range(1, NUM_EPOCHS + 1):
            current_alpha1 = min(1.0, epoch / NUM_EPOCHS)
            train_loss, train_acc, train_prec, train_rec, train_f1, train_auroc = train(model, train_loader, device, optimizer, epoch, label_names, current_alpha1)
        ...
        # # Evaluate training ROC curve (after training)
        # train_eval_loader = DataLoader(OdorDataset(smiles[train_idx], labels[train_idx]), batch_size=BATCH_SIZE, shuffle=False, collate_fn=collate_fn)
        # _, _, _, _, _, train_y_true, _, train_y_prob = evaluate(model, train_eval_loader, device, label_names)
        # plot_roc_curve(train_y_true, train_y_prob, label_names, title=f"Train ROC Curve - Fold {fold}")
        ...
        # plot_roc_curve(y_true, y_prob, label_names, title=f"Validation ROC Curve - Fold {fold}")
    ...
    print(f"\nTrain AUROC (micro) across {N_SPLITS} folds:")
